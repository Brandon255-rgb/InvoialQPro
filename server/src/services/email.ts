  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Please find attached your invoice #${invoice.invoiceNumber} for $${Number(invoice.total).toFixed(2)}.</p>
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Dear ${client.name},</p>
        <p>We have received your payment of $${Number(invoice.total).toFixed(2)} for invoice #${invoice.invoiceNumber}.</p>
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Dear ${client.name},</p>
        <p>This is a friendly reminder that invoice #${invoice.invoiceNumber} for $${Number(invoice.total).toFixed(2)} is due on ${new Date(invoice.dueDate).toLocaleDateString()}.</p>
        <p>You can view and pay this invoice online by clicking the button below:</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${process.env.CLIENT_PORTAL_URL}/invoices/${invoice.id}" 
             style="background-color: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px;">
...
export async function sendPaymentFailureNotification(invoice: Invoice, client: Client): Promise<void> {
  const mailOptions = {
    from: process.env.EMAIL_FROM,
    to: client.email,
...
export async function sendTeamInviteEmail(email: string, inviteToken: string): Promise<void> {
  const inviteUrl = `${process.env.CLIENT_PORTAL_URL}/invite/${inviteToken}`;
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>You've been invited to join ${process.env.COMPANY_NAME}</h2>
        <p>You've been invited to join our team. Click the button below to accept the invitation:</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${inviteUrl}" 
             style="background-color: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px;">
            Accept Invitation
